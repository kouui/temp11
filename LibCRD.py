...
author  : kouui
date    : 2018-06
purpose : Functions for radiative transfer in CRD.
          we apply functional programing instead of object oriented programing
          for the implementation of the `numba` optimization.
"""
...
#@nb.jit(nb.float64[:](nb.float64[:],nb.float64[:],nb.float64[:],nb.float64[:],nb.float64[:]), nopython=True)
def DirectFeautrier1(tau, I_upper, I_lower, planckB, eps):
    """
...
def FormalFeautrier2nd(tau, S, mu, I_upper, I_lower, method="elimination"):
    """
...
def FormalImprovedRH(tau, S, mu, r0=0, h0=0, rn=0, hn=0):
    """
    Purpose :
...
def TmatrixImprovedRH(tau, mu, r0=0, rn=0):
    """
    Purpose :
...
def AngleAveragedTmatrix_v0(TmatirxFunc, tau, r0=0, rn=0):
    """
    !!! Warning: Don't Use !!!
    ...
    Tmats = np.empty((4,ND,ND), dtype=np.double)
    for i in range(4):
        Tmats[i,:,:] = TmatirxFunc(tau, mus[i], r0=r0, rn=rn)
        T_average[:,:] += ws[i] * Tmats[i,:,:]

    return T_average, Tmats

def AngleAveragedTmatrix(TmatirxFunc, tau, I_upper=np.zeros(4,dtype=np.double), I_lower=np.zeros(4,dtype=np.double), r0=0, rn=0):
    """
    Purpose :
        Given a function to compute monochromatic T matrix of lambda transformation,
        calculate its 4 point Angle Quadrature
